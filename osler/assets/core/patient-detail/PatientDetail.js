      ...
      <div>
          ...
          <div className="container">
            <div className="col-lg-5 col-lg-offset-1 col-sm-6">
              <div className="col-md-1">
                <h2><br /><a href="/core/all/"><span className="glyphicon glyphicon-chevron-left" aria-hidden="true"></span></a></h2>
              </div>
              <div className="col-md-11">
                <h2> <a href={data["update_url"]}>{data["name"]}</a></h2>
                <p className="lead">{data["age"]} y/o  {data["ethnicities"].join(", ")}  {data["gender"].toLowerCase()}</p>
                <p className="lead"><strong>Action Items:</strong> {data["actionitem_status"]} </p>
                {props.displayReferrals && 
                  <p className="lead"><strong>FQHC Referral Status:</strong>  referral_status </p>
                }
                {props.displayReferrals && 
                  <p className="lead"><strong>Referrals:</strong>  referrals.iterator | join:", " </p>
                }
                {props.displayCaseManagers && 
                  <p className="lead"><strong>Case Manager:</strong> patient.case_managers.iterator | join:"; "</p>
                }
                <p className="lead"> <strong>Status:</strong> {data["status"]}
                % if can_activate %
                <a href={data["activate_url"]}><span className="glyphicon glyphicon-remove-circle" aria-hidden="true"></span></a>
                % endif %</p>
                {data["pending_workup_set"].length > 0 &&
                  <div className="alert alert-danger" role="alert">
                    Patient has a <a className="alert-link" href="% url 'workup' workup.pk %}">pending workup.</a>
                  </div>
                } 
              </div>
            </div>
        ...
        <div className="container">
          <div className="col-md-8 col-md-offset-2">
            {data["demographics"] ? 
            <a href="{% url 'demographics-detail' patient.demographics.id %}" className="btn btn-default" role="button">See Patient Survey Data</a>
            :
            <div className="alert alert-danger" role="alert">
              No survey data exists for this patient. Please <a className="alert-link" href={data["demographics_create_url"]}>click here <span className="glyphicon glyphicon-pencil" aria-hidden="true"></span></a> to add it.</div>
            }
          </div>
        </div>
        <div className="container">
